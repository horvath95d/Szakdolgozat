...
class Ion_auth_model extends CI_Model
{
    ...
    public function get_all_school() {
        $query = $this->db->query("SELECT `id`, `full_name`, `short_name` FROM `school`");
    ...
    public function register_user() {
        ...
        if (isset($_POST['checkbox'])) {
            ...
            if ($_POST['code'] == $this->get_school($_POST['full_name'])['code'])
                $school_id = $_POST['full_name'];

            else {
                $this->session->set_flashdata('message', 'Hibás biztosnági kód az iskolához!');
                return false;
            }
            ...
            $this->db->insert('school', $record);

			$school_id = $this->get_record('school', $record)['id'];
        ...
        $record = array(
            ...
            'active_end' => date('Y-m-d', strtotime(' + '.$_POST['active'].' months'))
        );

        $this->db->insert('users', $record);
        
        if (!isset($_POST['checkbox'])) {
            $owner_id = $this->get_record('users', $record)['id'];
            $this->db->query("UPDATE `school` SET `owner_id`=".$owner_id." WHERE `id`=".$school_id);
        }

        $this->session->set_flashdata('message', 'Sikeres regisztráció!');
        return true;
    }
}
